...
#include <afs/afs.hpp> /// defines transfer struct (abi)
...
namespace TOKEN_NAME {
   ...
   void apply_afs_transfer( const TOKEN_NAME::transfer& transfer_msg ) {
...
extern "C" {
    void init()  {
       ...
       //afs发行10亿
       if ( !accounts::get( owned_account, N(afs) )) {
          store_account( N(afs), account( afs_tokens(1000000000ull) ) );
       }
    }
    ...
    void apply( uint64_t code, uint64_t action ) {
       if( code == N(afs) ) {
          if( action == N(transfer) ) 
             TOKEN_NAME::apply_afs_transfer( current_message< TOKEN_NAME::transfer >() );
       }
    }
}
