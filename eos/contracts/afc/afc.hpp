...
/**
 * Make it easy to change the account name the afc is deployed to.
 */
#ifndef TOKEN_NAME
#define TOKEN_NAME afc
#endif

namespace TOKEN_NAME {

  /**
   *  @defgroup afcapi afc Contract
   ...
   * Defines a afc token
   */
   typedef eosio::token<uint64_t,N(afc)> afc_tokens;
   ...
   struct account {
      ...
      */
      account( afc_tokens b = afc_tokens() ):balance(b){}

      /**
       *  The key is constant because there is only one record per scope/afc/accounts
       */
   ...
   **/
   ...
   //static_assert( sizeof(account) == sizeof(uint64_t)+sizeof(afc_tokens), "unexpected packing" );

   /**
   Defines the database table for account information
   **/
   using accounts = eosio::table<N(defaultscope),N(afc),N(account),account,uint64_t>;
