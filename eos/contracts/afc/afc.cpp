...
#include <afc/afc.hpp> /// defines transfer struct (abi)
...
namespace TOKEN_NAME {
   ...
   void apply_afc_transfer( const TOKEN_NAME::transfer& transfer_msg ) {
...
extern "C" {
    void init()  {
       ...
       //afc发行1亿
       if ( !accounts::get( owned_account, N(afc) )) {
          store_account( N(afc), account( afc_tokens(100000000ull) ) );
       }
    }
    ...
    void apply( uint64_t code, uint64_t action ) {
       if( code == N(afc) ) {
          if( action == N(transfer) ) 
             TOKEN_NAME::apply_afc_transfer( current_message< TOKEN_NAME::transfer >() );
       }
    }
}
